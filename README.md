...
> 视频完整讲解并演示了全部核心功能。在线体验部署在海外，**国内访问需科学上网**；若打不开网站，看本视频即可了解完整产品。
...
| 功能 | 说明 |
|---|---|
| 多场景对话 | 日常（面试/点餐/会议/医院/电话/客服）+ 程序员面试（行为/项目/思维）；难度可选 |
| 人格化面试官 | 😊 友好 / 🔥 严厉 / 😐 冷面 三种 AI 风格，给不同压力体验 |
| 流式 AI 回复 | 回复逐字生成显示并朗读，首字延迟低、体验丝滑 |
| 拟真音色 TTS | Azure 神经网络语音，自然带情感（失败自动回退浏览器 TTS） |
| 苏格拉底式追问 | 对空洞/敷衍的回答连环追问，逼出具体细节 |
| 自适应难度 | 按用户实时表现动态微调 AI 用词难度（最近发展区） |
| 母语桥梁 | 可中英混合打字，不会的词用中文，AI 教你地道英文说法 |
| 对话录音回放 | 回放自己每句录音 + 重听 AI 句子 |

### 📊 评测与反馈
| 功能 | 说明 |
|---|---|
| 发音三维度评测 | Accuracy / Fluency / Expression + 具体改进建议 |
| 音素级评估 | 定位读错的**具体音素**（如 th→s、l/r 混淆） |
| 影子跟读 Shadowing | 听标准 → 跟读 → Azure 逐词染色评分 |
| 实时纠错 | 语法/词汇错误「原句 → 改后」对照与解释 |
| 表达升级 + 风格标签 | 把生硬/中式表达改成地道说法；标注 Confident/Hesitant 等风格 |
...
| 功能 | 说明 |
|---|---|
| 记忆闭环 | 生词收藏 → AI 在后续对话中自然复现，巩固记忆 |
| 生词本 | 收藏的生词 + 释义 + 点击发音 |
| 成长仪表盘 | 练习次数 / 平均分 / 最高分 / 连续打卡 + 多维趋势曲线 |
| 跨会话记忆 | 记住上次薄弱项，下次开场回顾并针对训练 |
| 选词查义 / 选句翻译 | 选词看音标释义、选句看中文翻译，均可 🔊 朗读 |
